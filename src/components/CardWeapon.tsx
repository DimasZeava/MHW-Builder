...
const CardWeapon: React.FC<CardWeaponProps> = ({type}) => {
  ...
  return (
    <div>
      <div
        className="card flex items-center gap-5 w-2xl p-4 bg-zinc-900 rounded-lg shadow-lg m-4 cursor-pointer h-36"
        onClick={openModal}
      >
        <div className="icon-armor p-2 bg-gray-800 rounded-lg shadow-lg">
          <img src={weaponTypeIcons[type]} alt={type} className="icon" />
        </div>
        <div className="label-armor text-center text-xl font-bold text-zinc-100">On Progress</div>
      </div>
